...
def simulate_scenario(df, apply_incentive=False, improved=False):
    learners = [initial_learners]
    for i in range(1, duration):
        rate = df.loc[i, 'dropoff_rate'] if i < len(df) else df['dropoff_rate'].mean()
        if apply_incentive and (i + 1) == incentive_month:
            if improved:
                rate *= (1 - retention_boost / 100)
            # else: rate remains the same but incentive cost applies
        retained = learners[-1] * (1 - rate / 100)
        learners.append(retained)
    return learners

scenarios = {
    "Baseline (no incentive)": simulate_scenario(data, apply_incentive=False, improved=False),
    "Incentive, no effect": simulate_scenario(data, apply_incentive=True, improved=False),
    "Incentive, improved retention": simulate_scenario(data, apply_incentive=True, improved=True)
}

# Calculate Metrics
def calculate_metrics(learners, apply_incentive):
    gross_revenue = sum([l * revenue_per_learner for l in learners])
    incentive_total = learners[incentive_month - 1] * incentive_cost if apply_incentive and 1 <= incentive_month <= len(learners) else 0
...
for key, learners in scenarios.items():
    apply_incentive = "Incentive" in key
    improved = "improved" in key
    gross, cost, net = calculate_metrics(learners, apply_incentive)
    metrics[key] = {
        "learners": learners,
        "gross": gross,
        "cost": cost,
